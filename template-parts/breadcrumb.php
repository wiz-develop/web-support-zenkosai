...
function lightning_bread_crumb() {
	...
	$get_page = get_page(get_the_ID());
	$get_page_slug = $get_page->post_name;
	if ('ufaq' === $post_type || 'keyword' === $get_page_slug) {
		$breadcrumb_html .= '<div>';
		$breadcrumb_html .= '<div class="faq_breadcrumb_container">';
	} else {
	...
	/*-------------------------------*/
		...
		if ( 'post' === $post_type && 'page' === $show_on_front && $page_for_post ) { /* including single-post */
			...
			$breadcrumb_html .= '<li' . $microdata_li . '>';
			if ('ufaq' === $post_type) {
				$breadcrumb_html .= '<a' . $microdata_li_a . ' href="/faq/">';
				$breadcrumb_html .= '<span' . $microdata_li_a_span . '>FAQ</span>';
			} else {
